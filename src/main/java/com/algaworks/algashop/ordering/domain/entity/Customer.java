...
public class Customer {
    ...
    public void addLoyaltyPoints(Integer points) {
        verifyIfChangeable();
        if (points <= 0) {
            throw new IllegalArgumentException("Loyalty points must not be lower than or equal to zero");
        }
    ...
    public OffsetDateTime archievedAt() {
        return archivedAt;
    }
    ...
    private void setArchived(Boolean archieved) {
        Objects.requireNonNull(archieved);
    ...
    private void setLoyaltyPoints(Integer loyaltyPoints) {
        ...
        if (loyaltyPoints < 0) {
            throw new IllegalArgumentException("Loyalty points must not be lower than zero");
        }
        this.loyaltyPoints = loyaltyPoints;
    }

    private void verifyIfChangeable() {
        if (Boolean.TRUE.equals(this.isArchieved())) {
